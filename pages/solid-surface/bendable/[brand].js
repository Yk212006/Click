...
import { Box, Container, Heading, Text, Button, Grid, Breadcrumb, BreadcrumbItem, BreadcrumbLink, Divider, Badge, Stack } from "@chakra-ui/react";
    ...
    <Breadcrumb spacing="8px" separator={<ChevronRightIcon color="gray.500" />}>
      ...
      <BreadcrumbItem>
        <Link href="/solid-surface/bendable">
          <BreadcrumbLink>Bendable</BreadcrumbLink>
        </Link>
      </BreadcrumbItem>
...
const brandLogoMap = {
  "lx-hausys": "/home/lx.png",
  "staron": "/home/staron.png",
  "dupont": "/home/dupont.png",
  "tristone": "/home/tristone.png",
  "dunes": "/home/dunes.png",
  "rehau": "/home/rehau.png",
  "vivanta": "/home/vivanta.png",
  "vivantha": "/home/vivanta.png",
  "riya-brand": "/home/riya.png"
};

// Static brand list for fallback
const staticBrands = ["lx-hausys", "staron", "dupont", "tristone", "dunes", "rehau", "vivanta", "vivantha"];

export default function BendableBrandPage({ brandData }) {
        ...
        <Container maxW="7xl">
          <Grid templateColumns={{ base: "1fr", md: "1fr 1fr" }} gap={12} alignItems="start">
            ...
            <Box>
              ...
              <ProductGallery productPath={`${brand}-solid-surface`} />

              {/* Brand Logos */}
              <Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)", lg: "repeat(4, 1fr)" }} gap={4}>
                {brand && brandLogoMap[brand] && (
                  <Box
                    key={brand}
                    display="flex"
                  ...
                  >
                    <Image
                      src={brandLogoMap[brand] || "/logo-placeholder.png"}
                      alt={`${titleCaseSlug(brand)} logo`}
                      height={60}
                      width={120}
                      style={{ 
                        objectFit: "contain",
            ...
            <Box>
              ...
              <Text color="gray.600" fontSize="18px" mb={6} lineHeight="1.8">
                {brandData?.description || `${brandName} bendable solid surface. Premium thermoformable material for custom curved designs and architectural applications.`}
              </Text>
...
export async function getStaticPaths() {
  ...
  let slugs = [];
  try {
    const res = await fetch(`${API_BASE}/brands?filters[type][$eq]=bendable&pagination[pageSize]=100`);
    const json = await res.json();
    if (json && Array.isArray(json.data)) {
      slugs = json.data.map((item) => item.attributes?.slug || item.attributes?.name?.toLowerCase().replace(/\s+/g, "-") || String(item.id));
    }
  } catch (err) {
    console.error("getStaticPaths bendable error:", err.message || err);
  }
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    console.error("Error fetching bendable brand data:", err.message || err);
  }
